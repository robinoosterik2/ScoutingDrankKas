...
export default defineEventHandler(async (event) => {
    ...
    if (await User.findOne({ "firstName": normalizedFirstName, "lastName": normalizedLastName })) {
        throw createError({ statusCode: 400, statusMessage: "User already has an account" });
    }
