<template>
        ...
        <form @submit.prevent="createCustomRole" class="space-y-2">
          ...
          <div>
            ...
            <textarea 
              ...
              rows="3"
            ></textarea>
            <div id="errorRoleDescription" class="text-red-500 text-sm mt-1"></div>
          </div>
            ...
            >
              ...
              >
              <label :for="permission" class="text-sm text-gray-700 dark:text-gray-300">
                {{ permission }}
              </label>
            </div>
            <div id="errorRolePermissions" class="text-red-500 text-sm mt-1"></div>
          </div>
  ...
  const createCustomRole = async () => {
    ...
    if (!roleForm.value.roleName) {
      document.getElementById('errorRoleName').textContent = 'Role name is required'
      hasError = true
    }
  
    if (!roleForm.value.roleDescription) {
      document.getElementById('errorRoleDescription').textContent = 'Role description is required'
      hasError = true
    }
    ...
    try {
      ...
      if (error.statusMessage === 'Role name already exists') {
        ...
        console.error(error)
        document.getElementById('errorRoleName').textContent = 'Failed to create role'
      }
    }
  }
  </script>