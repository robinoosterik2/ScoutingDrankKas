...
<template>
    <div v-if="show" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
      <div class="bg-white dark:bg-gray-800 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div class="p-4 overflow-y-auto max-h-[60vh]">
          <div v-if="selectedProducts.length === 0" class="text-gray-500 dark:text-gray-400 text-center py-4">
  ...
  const selectedProducts = computed(() => {
    return props.products.filter(product => props.productCounts[product.id] > 0);
  });
