  ...
  async loginUser(
    ...
    try {
      ...
      const sessionCookie = response.headers?.["set-cookie"]?.find(
        (cookie: string) => cookie.startsWith("nuxt-session")
      );
...
): Promise<{ user: TestUser; session: AuthenticatedSession }> {
...
export async function createAuthenticatedAdmin(): Promise<{
  user: TestUser;
...
export async function createAuthenticatedStam(): Promise<{
  user: TestUser;
  session: AuthenticatedSession;
}> {
  const helper = getAuthTestHelper();
