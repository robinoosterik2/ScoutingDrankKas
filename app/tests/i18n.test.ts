...
function getAllFiles(dir: string): string[] {
	...
	return entries.flatMap(entry => {
		...
		if (entry.isDirectory() && !['node_modules', '.output', '.nuxt', '.vscode'].includes(entry.name)) {
...
describe('Translation key usage vs locale definitions', () => {
	...
	// const allKeysDefined = new Set(Object.keys(allLocales[`${baseLang}.json`] || {}));
	...
	localeFiles.forEach(file => {
		...
		it(`should not use translation keys that are missing from ${file}`, () => {
			...
			if (missingKeys.length > 0) {
				...
				throw new Error(`Missing ${missingKeys.length} translation key(s) in ${file}`);
			}
		...
		it(`should not have unused keys in ${file}`, () => {
			...
			if (unusedKeys.length > 0) {
				...
				throw new Warning(`Found ${unusedKeys.length} unused translation key(s) in ${file}`);
			}
