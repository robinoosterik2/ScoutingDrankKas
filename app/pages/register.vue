<template>
  <div class="flex items-center justify-center">
    <div
      class="w-full max-w-sm p-4 space-y-2 bg-white rounded-lg shadow-md dark:bg-gray-800"
    >
      <h2
      ...
      <form class="space-y-1" @submit.prevent="handleRegister">
        <div class="flex space-x-2">
          <div class="w-1/2">
            ...
            <input
              ...
              type="text"
              required
              class="relative block w-full px-3 py-1 text-white placeholder-gray-500 border border-gray-300 rounded-md appearance-none text-2xs focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
          ...
          <div class="w-1/2">
            ...
            <input
              ...
              type="text"
              required
              class="relative block w-full px-3 py-1 text-white placeholder-gray-500 border border-gray-300 rounded-md appearance-none focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
        ...
        <div>
          <label class="text-xs" for="username">{{
            $t("register.username")
          }}</label>
          <input
            id="username"
        ...
        <div>
          <label class="text-xs" for="confirmPassword">{{
...
const handleRegister = async () => {
  ...
  if (password.value !== confirmPassword.value) {
    document.getElementById("errorConfirmPassword").textContent = t(
      "register.errors.passwordsDoNotMatch"
    );
    return;
  }
  ...
  try {
    ...
    if (statusMessage === "Username and password are required") {
      ...
      document.getElementById("errorPassword").textContent = t(
        "register.errors.credentialsRequired"
      );
    } else if (statusMessage === "Username already exists") {
      document.getElementById("errorUsername").textContent = t(
        "register.errors.usernameAlreadyExists"
      );
    } else if (statusMessage === "Email already exists") {
      // Backend uses "Email already exists"
      document.getElementById("errorEmail").textContent = t(
        "register.errors.emailInUse"
      ); // Frontend key is emailInUse
    } else if (statusMessage === "User already has an account") {
      document.getElementById("errorLastName").textContent = t(
        "register.errors.userAlreadyExistsWithFullName"
      );
    } else if (statusMessage === "Password must be at least 8 characters") {
      document.getElementById("errorPassword").textContent = t(
        "register.errors.passwordTooShort"
      );
    } else if (statusMessage === "Passwords do not match") {
      // This is also checked client-side, but good to handle backend error too
      document.getElementById("errorConfirmPassword").textContent = t(
        "register.errors.passwordsDoNotMatch"
      );
    } else {
