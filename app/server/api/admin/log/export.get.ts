...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (users) {
      ...
      if (userIds.length > 0) {
        mongoQuery.executor = { $in: userIds.map(id => new mongoose.Types.ObjectId(id)) };
      }
    }
    ...
    if (format === 'csv') {
      ...
      event.res.setHeader('Content-Disposition', 'attachment; filename=logs_export.csv');
      
      // Generate CSV content
      const csvHeader = 'id,executor,action,objectType,objectId,description,createdAt\n';
      const csvRows = logs.map(log => {
        ...
        return `${log._id},${executor},${log.action},${log.objectType},${log.objectId?._id || log.objectId || ''},${description},${createdAt}`;
      });
      ...
      event.res.setHeader('Content-Disposition', 'attachment; filename=logs_export.json');
